def turbine_report(run, engine,
    ...
    if turbine['type'] == 'radial':
        ...
        if 'TYPE1' in run['type']:
            ...
            r.write("- В соответствии с исходными данными домашнего задания имеем:\n\
                    \n$$\n G_{K} = %0.4f кг/с \n$$\n Также принимаем:\
        ...
        if (Turbine.pressureRelationSetOfNozzles >= 0.54):
            r.write(", которое больше 0.54, поэтому применяем конфузорный\
                    сопловой аппарат\n\n")
        elif (Turbine.pressureRelationSetOfNozzles > 0.3 
              and Turbine.pressureRelationSetOfNozzles < 0.54):
        ...
        if (Turbine.M_c1 > 0.4
            and Turbine.pressureRelationSetOfNozzles < 0.6):

def report(run, engine, compressor, CompCalc):
    ...
    \n$$\n \pi_{к} = %3.4f, \n$$\n\n" %(p_e_MPa, compressor['G_K'], CompCalc.D_2Init*1e+03, compressor['pi_K'])
    )
    ...
    u2 ≤ 450…550 м/с.\n\n"
    %(CompCalc.D_2Init*1e+03, compressor['efficiency']['H_KsStagn'], CompCalc.u_2))
    ...
    %(compressor['losses']['dzeta_inlet'], CompCalc.L_inlet))
    ...
    %(CompCalc.D_2Init*1e+03, compressor['geometry']['coefficients']['relD_1B'], compressor['geometry']['coefficients']['relD_1H'], CompCalc.D_1H) )
    ...
    %(compressor['geometry']['iDeg'], CompCalc.beta_1Blade) )
    report.write(
    ...
    %(compressor['load']['tau_1'], CompCalc.c_1Tau) )
    ...
    %(compressor['losses']['dzeta_BA'], CompCalc.L_BA) )
    ...
    %(compressor['geometry']['coefficients']['relW_2rToC_1a'], CompCalc.c_2r) )
    ...
    %(compressor['losses']['dzeta_TF'], CompCalc.L_TF) )
    ...
    выберем _α_ = %1.3f.\n\n"
    %(CompCalc.L_TB, compressor['losses']['alpha_wh']) )
    ...
    %(CompCalc.b_2, compressor['load']['tau_2']) )
    ...
    if 'VANELESS' in compressor['diffuser']: # Vaneless diffuser | БЛД
        ...
        %(compressor['geometry']['coefficients']['vanelessWideCoef'], compressor['geometry']['coefficients']['vanelessWideCoef'], CompCalc.b_4) )
        ...
        %(compressor['geometry']['coefficients']['vanelessDiamCoef'], compressor['geometry']['coefficients']['vanelessDiamCoef'], CompCalc.D_4) )
        report.write(
        ...
        %(compressor['efficiency']['eta_diff'], CompCalc.n_4) )
        ...
        %(compressor['geometry']['coefficients']['relDiffOutToCompOut'], compressor['geometry']['coefficients']['relDiffOutToCompOut'], CompCalc.c_K) )
        i = 52
        ...
        %(compressor['geometry']['coefficients']['vanelessWideCoef'], compressor['geometry']['coefficients']['vanelessWideCoef'], CompCalc.b_3) )
        ...
        %(compressor['geometry']['coefficients']['vanelessDiamCoef'], compressor['geometry']['coefficients']['vanelessDiamCoef'], CompCalc.D_3) )
        ...
        %(compressor['efficiency']['eta_diff'], CompCalc.n_3) )
        report.write("47. Температура на выходе из диффузора определяется методом последовательных\
        приближений из уравнения:\
        \n$$\n {1 \over \\beta^{m}} + {\\beta - 1 \over \sigma q} -\
        ...
        %(compressor['geometry']['coefficients']['vanedWideCoef'], compressor['geometry']['coefficients']['vanedWideCoef'], CompCalc.b_4) )
        report.write("52. Наружный диаметр лопаточной части диффузора выбирают в пределах $$(1.35…1.7)D_{2}$$.\
        Принимаем значение коэффициента %0.2f. В результате:\
        \n$$\n D_{4} = %0.2f D_{2} = %1.4f\quad м\n$$\n\n"
        %(compressor['geometry']['coefficients']['vanedDiamCoef'], compressor['geometry']['coefficients']['vanedDiamCoef'], CompCalc.D_4) )
        report.write(
        "![alt text](compressor/axisCut.png)\n\
        <center>\
        ...
        %(compressor['geometry']['deltaDiffuser'], CompCalc.alpha_4) )
        ...
        %(compressor['load']['tau_3'], compressor['load']['tau_4'], CompCalc.T_3))

# -*- coding: utf-8 -*-
# '''
#     Description:    Create the report for a compressor w/ vaned diffuser
# '''
...
if 'ON' in roundDiamToSTD:  report.write("15. Наружный диаметр колеса компрессора на выходе,\
...
report.write("44. Ширина безлопаточной части диффузора на выходе может изменяться в пределах\
 ...
 %(vanelessWideCoef, vanelessWideCoef, b_3) );
report.write("45. Наружный диаметр безлопаточной части диффузора выбирают в пределах $$(1.6…1.9)D_{2}$$.\
 ...
 %(vanelessDiamCoef, vanelessDiamCoef, D_3) );
...
report.write("61. Температура на выходе из компрессора:\
 \n$$\n T_{4}^{*} = T_{4} + {c_{4}^{2} - c_{K}^{2} \over 2c_{p}} = %0.3f\quad К\n$$\n\n"
 %T_K);
report.write("62. Показатель $$n_{ул}$$ политропы сжатия в воздухосборнике (улитке) составляет обычно\
 1.85…2.2. Примем значение $$n_{ул} = %0.1f$$.\n\n"
 %n_housing);
report.write("63. Тогда давление на выходе из компрессора:\
 \n$$\n p_{K} = p_{4} \left( {T_{K} \over T_{4}} \\right)^{n_{ул} \over n_{ул}-1}\
 = %1.f \quad Па \n$$\n\n"
 %p_K);
report.write("64. Температура заторможенного потока на выходе:\
 \n$$\n T_{K}^{*} = T_{K} + {c_{K}^{2} \over 2c_{p}} = %0.3f\quad К\n$$\n\n"
 %T_KStagn);
report.write("65. Давление заторможенного потока на выходе:\
 \n$$\n p_{K}^{*} = p_{K} \left( {T_{K}^{*} \over T_{K}} \\right)^{k \over k-1}\
 = %1.f \quad Па \n$$\n\n"
 %p_KStagn);
report.write("66. Действительная степень повышения давления компрессоре:\
 \n$$\n \pi_{K} = {p_{K}^{*} \over p_{0}^{*}} = %0.4f \n$$\n\n"
 %Pi_KStagn);
report.write("67. Изоэнтропная работа по расчетной степени повышения давления:\
 \n$$\n {L'}_{КS}^{*} = c_{p}T_{0}^{*} \left( (\pi_К^{*})^{k \over k-1} - 1\\right)\
 = %0.1f \quad Дж/кг \n$$\n\n"
 %L_KsStagn);
report.write("68. Расчетный изоэнтропный КПД по заторможенным параметрам:\
 \n$$\n {\eta'}_{КS}^{*} = {(\pi_К^{*})^{k \over k-1} - 1   \over\
 T_{K}^{*}/T_{0}^{*} - 1} = %0.5f \n$$\n\n"
 %eta_KsStagnRated);
report.write("69. Расхождение с заданным КПД компрессора составляет:\
 \n$$\n {\mid {\eta'}_{КS}^{*} - \eta_{КS}^{*} \mid \over \eta_{КS}^{*}}\
 = %0.3f \%% \n$$\n\n"
 %differenceEta);
report.write("70. Расчетный коэффициент напора по заторможенным параметрам:\
 \n$$\n {H'}_{КS}^{*} = {{L'}_{КS}^{*} \over u_{2}^2} = %0.5f \n$$\n\n"
 %H_KsStagnRated);
report.write("71. Расхождение с заданным коэффициентом напора составляет:\
 \n$$\n {\mid {H'}_{КS}^{*} - H_{КS}^{*} \mid \over H_{КS}^{*}}\
 = %0.3f \%% \n$$\n\n"
 %differenceH);
report.write("72. Мощность, затрачиваемая на привод компрессора:\
 \n$$\n N_{K} = {G_{K}{L'}_{КS}^{*} \over {\eta'}_{КS}^{*}} = %0.1f\quad Вт \n$$\n\n"
 %N_K);
report.write("73. Полное давление перед впускными клапанами поршневой части:\
